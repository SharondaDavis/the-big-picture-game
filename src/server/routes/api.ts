...
api.get('/game-state', async (c) => {
  ...
  const [canvas, hand, triesStr, scoreStr, streakStr, leaderboard, completeStr] =
    await Promise.all([
      getCanvas(today),
  ...
  const triesLeft = triesStr !== undefined ? parseInt(triesStr) : 3;
...
api.post('/place', async (c) => {
  ...
  const body = await c.req.json<{ pieceId: number; cellIndex: number }>();
  ...
  if (correct) {
    ...
    const newScore = await redis.incrBy(K.score(today, username), 1);
    ...
    const newTries = Math.max(0, await redis.incrBy(K.tries(today, username), -1));
    const scoreStr = await redis.get(K.score(today, username));
    const canvas = await getCanvas(today);
